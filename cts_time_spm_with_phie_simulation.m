...
load('phie_sysid_Aug_07_2018_16_14_58'); % load overpotential results for phie
run('user_inputs_for_sim.m');
run('pre_process_script.m');
run('cts_spm_core_sim'); % spm simulation loop is in this script
% return;
v_cell_sim_results_spm = v_cell_sim_results_spm + (phie_op_vector_results(1:length(v_cell_sim_results_spm)))';
%% Save results to disk
save_foldername = ['spm_results_with_phie/', cellIdentifier, '/', load_profile_name];
