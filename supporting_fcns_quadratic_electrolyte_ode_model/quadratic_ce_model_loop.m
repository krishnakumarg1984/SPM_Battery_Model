...
quadratic_ce_sim_time_vector(1) = t0_local_quadratic;
load_current_vector        = nan(num_iterations,1);
sim_results_quadratic_ce = nan(num_iterations,1);
load_current_vector(1) = I_1C*(interp1(C_rate_profile(:,1),C_rate_profile(:,2),t0_local_quadratic,'previous','extrap'));
%% Run PP2D model simulation loop
progressbarText(0);
step_quadratic = 2;
while step_quadratic < num_iterations
    I_load_t = I_1C*(interp1(C_rate_profile(:,1),C_rate_profile(:,2),tf_local_quadratic,'previous','extrap'));
    ...
    a_quadratic_vector_results(:,step_quadratic) = a_quadratic_vector;  % required for later post-processing
    Qe_quadratic_vector_results(:,step_quadratic) = Qe_quadratic_vector_init; % Only for checking if an 8-th equation is possible or not
    
    quadratic_ce_sim_time_vector(step_quadratic) = tf_local_quadratic;
