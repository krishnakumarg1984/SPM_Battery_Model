...
% extradata_for_solver.Qen_tfest_scaled = Qen_tfest_scaled;
% extradata_for_solver.Qep_tfest_scaled = Qep_tfest_scaled;
Qen_ss_model = tf(Qen_tfest_scaled);
Qep_ss_model = tf(Qep_tfest_scaled);
...
a_tf_vector_results = nan(length(a_tf_vector),num_iterations);
a_tf_vector_results(:,1) = a_tf_vector;
Qe_tf_vector_results = nan(length(Qe_tf_vector_init),num_iterations);
Qe_tf_vector_results(:,1) = Qe_tf_vector_init;

tf_ce_sim_time_vector = nan(num_iterations,1);
t0_local_tf = 0;
tf_local_tf = Ts;

tf_ce_sim_time_vector(1) = t0_local_tf;
load_current_vector      = nan(num_iterations,1);
sim_results_tf_ce = nan(num_iterations,1);
...
while step_tf < num_iterations
    ...
    Qen_vector_sol_tf = ((1/1000)*lsim(Qen_ss_model,I_load_t*ones(size(tspan_tf')),tspan_tf',[],'zoh')) + Qe_tf_vector_init(1);
    Qep_vector_sol_tf = ((1/1000)*lsim(Qep_ss_model,I_load_t*ones(size(tspan_tf')),tspan_tf',[],'zoh')) + Qe_tf_vector_init(3);
    Qe_tf_vector_init = [Qen_vector_sol_tf(end);Qes_vector_sol_tf(end);Qep_vector_sol_tf(end)];

    [a_tf_vector,~,~] = solve_algebraic_a_vector(extradata_for_solver,Qe_tf_vector_init);
    ...
    step_tf = step_tf + 1;
    progressbarText((step_tf-1)/num_iterations);
end
